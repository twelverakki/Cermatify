    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        if (peserta.sudahSelesai()) {
            ...
            for (Map.Entry<String, Peserta> entry : Main.hasilTesMap.entrySet()) {
                if (count >= 5) break;  // Tampilkan hanya 5 besar
                result += "<li>" + entry.getKey() + " - " + entry.getValue().getPoin() + " Poin</li>";
                count++;
            }
            result += "</ol>";
            result += "<p><button onclick=\"location.href='/submit_nama?nama="+ nama +"'\">Kembali ke Halaman Awal</button></p>";
            result += "</body></html>";

//p

            // Kirimkan response dengan hasil tes
